...
public class WeatherControl {
    ...
    @GetMapping(value = "/cityGreatThen/{maxTemp}", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<Weather> allMax(@PathVariable Integer maxTemp) {
        Flux<Weather> data = weathers.allMax(maxTemp);
        Flux<Long> delay = Flux.interval(Duration.ofSeconds(3));
        return Flux.zip(data, delay).map(Tuple2::getT1);
    }
